...
//  import pet.store.controller.error.GlobalControllerErrorHandler.ExceptionMessage;
//  import pet.store.controller.error.GlobalControllerErrorHandler.LogStatus;
...
public class GlobalErrorHandler {
	...
	public ExceptionMessage handleDupolicateKeyException(DuplicateKeyException ex, WebRequest webRequest)		{
	...
	private ExceptionMessage buildExceptionMessage(Exception ex, HttpStatus status, WebRequest webRequest, LogStatus logStatus) {
		...
		if(webRequest instanceof ServletWebRequest swr)	{	uri = swr.getRequest().getRequestURI();	}
		if(logStatus == LogStatus.MESSAGE_ONLY)	{	log.error("Exception: {}", ex.toString());	}
		else	{	log.error("Exception: ", ex);	}
